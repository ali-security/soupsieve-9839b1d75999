...
class CSSParser:
    ...
    def increment_count(self, increment: int = 1) -> None:
        """Increment the current selector count and check that we are still under the limit."""
    ...
    def parse_combinator(
        ...
        if not has_selector:
            ...
            # but count it so that we cannot be abused with a massive, empty selector list.
            if combinator == COMMA_COMBINATOR:
